...
def atrous_spatial_pyramid_pooling(net, scope, depth=256, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        feature_map_size = tf.shape(input=net)

        # apply global average pooling
        image_level_features = tf.reduce_mean(input_tensor=net, axis=[1, 2], name='image_level_global_pool', keepdims=True)
        image_level_features = slim.conv2d(image_level_features, depth, [1, 1], scope="image_level_conv_1x1",
                                           activation_fn=None)
        image_level_features = tf.image.resize(image_level_features, (feature_map_size[1], feature_map_size[2]), method=tf.image.ResizeMethod.BILINEAR)
