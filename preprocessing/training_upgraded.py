...
def get_labels_from_annotation(annotation_tensor, class_labels):
    ...
    labels_2d_stacked_float = tf.cast(labels_2d_stacked, dtype=tf.float32)
...
def get_valid_entries_indices_from_annotation_batch(annotation_batch_tensor, class_labels):
    ...
    valid_labels_indices = tf.compat.v1.where(valid_labels_mask)

    return tf.cast(valid_labels_indices, dtype=tf.int32)
