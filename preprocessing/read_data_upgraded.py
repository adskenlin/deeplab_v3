...
def random_flip_image_and_annotation(image_tensor, annotation_tensor, image_shape):
    ...
    """
    original_shape = tf.shape(input=annotation_tensor)
    # ensure the annotation tensor has shape (width, height, 1)
    annotation_tensor = tf.cond(pred=tf.rank(annotation_tensor) < 3,
                                true_fn=lambda: tf.expand_dims(annotation_tensor, axis=2),
...
def rescale_image_and_annotation_by_factor(image, annotation, image_shape, nin_scale=0.5, max_scale=2):
    ...
    input_shape = tf.shape(input=image)[0:2]
    input_shape_float = tf.cast(input_shape, dtype=tf.float32)

    scale = tf.random.uniform(shape=[1],
                                 minval=0.5,
                                 maxval=2)

    scaled_input_shape = tf.cast(tf.round(input_shape_float * scale), dtype=tf.int32)

    image = tf.image.resize(image, scaled_input_shape,
                                  method=tf.image.ResizeMethod.BILINEAR)

    # use nearest neighbour for annotations resizing in order to keep proper values
    annotation = tf.image.resize(annotation, scaled_input_shape,
...
def scale_image_with_crop_padding(image, annotation, image_shape, crop_size):
    ...
    image_croped = tf.image.resize_with_crop_or_pad(image,crop_size,crop_size)
    ...
    cropped_padded_annotation = tf.image.resize_with_crop_or_pad(annotation_shifted_classes,crop_size,crop_size)

    mask_out_number=255
    annotation_additional_mask_out = tf.cast(tf.equal(cropped_padded_annotation, 0), dtype=tf.int32) * (mask_out_number+1)
...
def tf_record_parser(record):
    keys_to_features = {
        ...
        'annotation_raw': tf.io.FixedLenFeature([], tf.string),
        "height": tf.io.FixedLenFeature((), tf.int64),
        "width": tf.io.FixedLenFeature((), tf.int64)
    }

    features = tf.io.parse_single_example(serialized=record, features=keys_to_features)

    image = tf.io.decode_raw(features['image_raw'], tf.uint8)
    annotation = tf.io.decode_raw(features['annotation_raw'], tf.uint8)
    ...
    return tf.cast(image, dtype=tf.float32), annotation, (height, width)
...
def distort_randomly_image_color(image_tensor, annotation_tensor, image_shape):
    ...
    img_float_zero_one_range = tf.cast(image_tensor, dtype=tf.float32) / 255
