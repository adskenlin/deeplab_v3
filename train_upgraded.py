...
training_iterator = tf.compat.v1.data.make_initializable_iterator(training_dataset)
validation_iterator = tf.compat.v1.data.make_initializable_iterator(validation_dataset)

class_labels = [v for v in range((args.number_of_classes+1))]
class_labels[-1] = 255

is_training_tf = tf.compat.v1.placeholder(tf.bool, shape=[])

logits_tf = tf.cond(pred=is_training_tf, true_fn= lambda: network.deeplab_v3(batch_images_tf, args, is_training=True, reuse=False),
...
cross_entropy_tf = tf.reduce_mean(input_tensor=cross_entropies)
predictions_tf = tf.argmax(input=logits_tf, axis=3)

tf.compat.v1.summary.scalar('cross_entropy', cross_entropy_tf)
...
with tf.compat.v1.variable_scope("optimizer_vars"):
    global_step = tf.Variable(0, trainable=False)
    optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=args.starting_learning_rate)
...
miou, update_op = tf.contrib.metrics.streaming_mean_iou(tf.argmax(input=valid_logits_batch_tf, axis=1),
                                                        tf.argmax(input=valid_labels_batch_tf, axis=1),
...
with tf.compat.v1.Session() as sess:
    ...
    train_writer = tf.compat.v1.summary.FileWriter(LOG_FOLDER + "/train", sess.graph)
    test_writer = tf.compat.v1.summary.FileWriter(LOG_FOLDER + '/val')
