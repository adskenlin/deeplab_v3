...
def main(_):
    ...
    with tf.compat.v1.Session(graph=tf.Graph()) as sess:
        ...
        image_height_tensor = tf.compat.v1.placeholder(tf.int32)
        image_width_tensor = tf.compat.v1.placeholder(tf.int32)
        ...
        feature_configs = {'x': tf.io.FixedLenFeature(shape=[], dtype=tf.float32), }
        ...
        tensor_info_height = tf.compat.v1.saved_model.utils.build_tensor_info(image_height_tensor)
        tensor_info_width = tf.compat.v1.saved_model.utils.build_tensor_info(image_width_tensor)

        # output tensor info
        tensor_info_output = tf.compat.v1.saved_model.utils.build_tensor_info(predictions_tf)
